"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateSingleUser = exports.deleteSingleUser = exports.getAllUser = exports.getSingleUser = exports.createUserIntoDB = void 0;
const user_service_1 = require("./user.service");
const createUserIntoDB = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const data = req.body;
    const user = yield (0, user_service_1.createUserToDB)(data);
    res.status(200).json({
        status: 'success',
        data: user
    });
    console.log('user created successful');
});
exports.createUserIntoDB = createUserIntoDB;
const getSingleUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const id = req.params.id;
    const user = yield (0, user_service_1.getUser)(id);
    res.status(200).json({
        status: 'success',
        data: user
    });
    console.log(`user fetched`);
});
exports.getSingleUser = getSingleUser;
const getAllUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const users = yield (0, user_service_1.getUsers)();
    res.status(200).json({
        status: 'success',
        data: users
    });
    console.log(`users fetched`);
});
exports.getAllUser = getAllUser;
const deleteSingleUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const id = req.params.id;
    const user = yield (0, user_service_1.deleteUser)(id);
    res.status(200).json({
        status: 'success',
        data: user
    });
    console.log('user deleted');
});
exports.deleteSingleUser = deleteSingleUser;
const updateSingleUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const id = req.params.id;
    const updatedData = req.body;
    const user = yield (0, user_service_1.updateUser)(id, updatedData);
    res.status(200).json({
        status: 'success',
        data: user
    });
    console.log('user updated');
});
exports.updateSingleUser = updateSingleUser;
